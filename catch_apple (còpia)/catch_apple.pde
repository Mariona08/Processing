...
void draw(){
  ...
  // Pinta la imatge de Newton amb tint verd si ha agafat una poma
  if (!pCount) {
    tint(0, 0, 0);  // tint verd
  } else {
    noTint();
  }
  ...
  if (timer >= 30) {
  background(255, 0, 0);  // fons vermell si el temps s'acaba
  fill(255);
  ...
  if (p == win) {
  background(0, 200, 0);  // fons verd si guanya
  fill(255);
  textSize(48);  // mida més gran
  textAlign(CENTER, CENTER);  // centrat al mig
